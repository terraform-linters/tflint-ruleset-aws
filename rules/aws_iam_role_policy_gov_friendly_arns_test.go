...
func Test_AwsIamRolePolicyGovFriendlyArns(t *testing.T) {
	cases := []struct {
		Name     string
		Content  string
		Config   string
...
rule "aws_iam_role_policy_gov_friendly_arns" {
  enabled = true
			...
			Expected: helper.Issues{
				{
					Rule:    NewAwsIAMRolePolicyGovFriendlyArnsRule(),
					Message: "ARN detected in IAM role policy that could potentially fail in AWS GovCloud due to resource pattern: arn:aws:.*",
					Range: hcl.Range{
						Filename: "module.tf",
