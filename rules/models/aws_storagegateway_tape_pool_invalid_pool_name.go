// This file generated by `generator/`. DO NOT EDIT

package models

import (
	"fmt"
	"log"
	"regexp"

	hcl "github.com/hashicorp/hcl/v2"
	"github.com/terraform-linters/tflint-plugin-sdk/tflint"
)

// AwsStoragegatewayTapePoolInvalidPoolNameRule checks the pattern is valid
type AwsStoragegatewayTapePoolInvalidPoolNameRule struct {
	resourceType  string
	attributeName string
	max           int
	min           int
	pattern       *regexp.Regexp
}

// NewAwsStoragegatewayTapePoolInvalidPoolNameRule returns new rule with default attributes
func NewAwsStoragegatewayTapePoolInvalidPoolNameRule() *AwsStoragegatewayTapePoolInvalidPoolNameRule {
	return &AwsStoragegatewayTapePoolInvalidPoolNameRule{
		resourceType:  "aws_storagegateway_tape_pool",
		attributeName: "pool_name",
		max:           100,
		min:           1,
		pattern:       regexp.MustCompile(`^[ -\.0-\[\]-~]*[!-\.0-\[\]-~][ -\.0-\[\]-~]*$`),
	}
}

// Name returns the rule name
func (r *AwsStoragegatewayTapePoolInvalidPoolNameRule) Name() string {
	return "aws_storagegateway_tape_pool_invalid_pool_name"
}

// Enabled returns whether the rule is enabled by default
func (r *AwsStoragegatewayTapePoolInvalidPoolNameRule) Enabled() bool {
	return true
}

// Severity returns the rule severity
func (r *AwsStoragegatewayTapePoolInvalidPoolNameRule) Severity() string {
	return tflint.ERROR
}

// Link returns the rule reference link
func (r *AwsStoragegatewayTapePoolInvalidPoolNameRule) Link() string {
	return ""
}

// Check checks the pattern is valid
func (r *AwsStoragegatewayTapePoolInvalidPoolNameRule) Check(runner tflint.Runner) error {
	log.Printf("[TRACE] Check `%s` rule", r.Name())

	return runner.WalkResourceAttributes(r.resourceType, r.attributeName, func(attribute *hcl.Attribute) error {
		var val string
		err := runner.EvaluateExpr(attribute.Expr, &val, nil)

		return runner.EnsureNoError(err, func() error {
			if len(val) > r.max {
				runner.EmitIssueOnExpr(
					r,
					"pool_name must be 100 characters or less",
					attribute.Expr,
				)
			}
			if len(val) < r.min {
				runner.EmitIssueOnExpr(
					r,
					"pool_name must be 1 characters or higher",
					attribute.Expr,
				)
			}
			if !r.pattern.MatchString(val) {
				runner.EmitIssueOnExpr(
					r,
					fmt.Sprintf(`"%s" does not match valid pattern %s`, truncateLongMessage(val), `^[ -\.0-\[\]-~]*[!-\.0-\[\]-~][ -\.0-\[\]-~]*$`),
					attribute.Expr,
				)
			}
			return nil
		})
	})
}
