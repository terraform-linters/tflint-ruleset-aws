...
func NewAwsServicecatalogBudgetResourceAssociationInvalidBudgetNameRule() *AwsServicecatalogBudgetResourceAssociationInvalidBudgetNameRule {
	return &AwsServicecatalogBudgetResourceAssociationInvalidBudgetNameRule{
...
func (r *AwsServicecatalogBudgetResourceAssociationInvalidBudgetNameRule) Name() string {
	return "aws_servicecatalog_budget_resource_association_invalid_budget_name"
}
...
func (r *AwsServicecatalogBudgetResourceAssociationInvalidBudgetNameRule) Severity() string {
	return tflint.ERROR
}
		...
		return runner.EnsureNoError(err, func() error {
			if len(val) > r.max {
				runner.EmitIssueOnExpr(
					r,
					"budget_name must be 100 characters or less",
					attribute.Expr,
				)
			}
			if len(val) < r.min {
				runner.EmitIssueOnExpr(
					r,
					"budget_name must be 1 characters or higher",
					attribute.Expr,
				)
			}
			return nil
