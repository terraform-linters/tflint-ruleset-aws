...
func NewAwsDatasyncTaskInvalidCloudwatchLogGroupArnRule() *AwsDatasyncTaskInvalidCloudwatchLogGroupArnRule {
	return &AwsDatasyncTaskInvalidCloudwatchLogGroupArnRule{
		...
		max:           562,
		pattern:       regexp.MustCompile(`^arn:(aws|aws-cn|aws-us-gov|aws-eusc|aws-iso|aws-iso-b):logs:[a-z\-0-9]+:[0-9]{12}:log-group:([^:\*]*)(:\*)?$`),
	}
}
	...
	for _, resource := range resources.Blocks {
		...
		err := runner.EvaluateExpr(attribute.Expr, func (val string) error {
			...
			if !r.pattern.MatchString(val) {
				runner.EmitIssue(
					r,
					fmt.Sprintf(`"%s" does not match valid pattern %s`, truncateLongMessage(val), `^arn:(aws|aws-cn|aws-us-gov|aws-eusc|aws-iso|aws-iso-b):logs:[a-z\-0-9]+:[0-9]{12}:log-group:([^:\*]*)(:\*)?$`),
					attribute.Expr.Range(),
				)
			}
			return nil
