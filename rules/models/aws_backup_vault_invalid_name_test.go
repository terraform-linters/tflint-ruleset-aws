// This file generated by `generator/`. DO NOT EDIT

package models

import (
	"testing"
	"fmt"

	"github.com/terraform-linters/tflint-plugin-sdk/helper"
)

func Test_AwsBackupVaultInvalidNameRule(t *testing.T) {
	cases := []struct {
		Name     string
		Content  string
		Expected helper.Issues
	}{
		{
			Name: "It includes invalid characters",
			Content: `
resource "aws_backup_vault" "foo" {
	name = "example_backup_vault_example_backup_vault_example_backup_vault"
}`,
			Expected: helper.Issues{
				{
					Rule:    NewAwsBackupVaultInvalidNameRule(),
					Message: fmt.Sprintf(`%q does not match valid pattern %s`, truncateLongMessage("example_backup_vault_example_backup_vault_example_backup_vault"), `^[a-zA-Z0-9\-\_]{2,50}$`),
				},
			},
		},
		{
			Name: "It is valid",
			Content: `
resource "aws_backup_vault" "foo" {
	name = "example_backup_vault"
}`,
			Expected: helper.Issues{},
		},
	}

	rule := NewAwsBackupVaultInvalidNameRule()

	for _, tc := range cases {
		runner := helper.TestRunner(t, map[string]string{"resource.tf": tc.Content})

		if err := rule.Check(runner); err != nil {
			t.Fatalf("Unexpected error occurred: %s", err)
		}

		helper.AssertIssuesWithoutRange(t, tc.Expected, runner.Issues)
	}
}
