...
func NewAwsEcrPullThroughCacheRuleInvalidEcrRepositoryPrefixRule() *AwsEcrPullThroughCacheRuleInvalidEcrRepositoryPrefixRule {
	return &AwsEcrPullThroughCacheRuleInvalidEcrRepositoryPrefixRule{
		...
		max:           30,
		min:           2,
		pattern:       regexp.MustCompile(`^([a-z0-9]+((\.|_|__|-+)[a-z0-9]+)*(\/[a-z0-9]+((\.|_|__|-+)[a-z0-9]+)*)*\/?|ROOT)$`),
	}
}
	...
	for _, resource := range resources.Blocks {
		...
		err := runner.EvaluateExpr(attribute.Expr, func (val string) error {
			...
			if !r.pattern.MatchString(val) {
				runner.EmitIssue(
					r,
					fmt.Sprintf(`"%s" does not match valid pattern %s`, truncateLongMessage(val), `^([a-z0-9]+((\.|_|__|-+)[a-z0-9]+)*(\/[a-z0-9]+((\.|_|__|-+)[a-z0-9]+)*)*\/?|ROOT)$`),
					attribute.Expr.Range(),
				)
			}
			return nil
