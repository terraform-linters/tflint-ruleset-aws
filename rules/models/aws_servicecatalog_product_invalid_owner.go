...
func NewAwsServicecatalogProductInvalidOwnerRule() *AwsServicecatalogProductInvalidOwnerRule {
	return &AwsServicecatalogProductInvalidOwnerRule{
...
func (r *AwsServicecatalogProductInvalidOwnerRule) Name() string {
	return "aws_servicecatalog_product_invalid_owner"
}
...
func (r *AwsServicecatalogProductInvalidOwnerRule) Severity() string {
	return tflint.ERROR
}
		...
		return runner.EnsureNoError(err, func() error {
			if len(val) > r.max {
				runner.EmitIssueOnExpr(
					r,
					"owner must be 8191 characters or less",
					attribute.Expr,
				)
			}
			return nil
